...
cu.execute('INSERT INTO EMAIL VALUES ("%s","%s","%s","%s","%s","3","%s","%s");'%(username,businessman,my_user,businesscode,mondaytime,businessname,remark))
cu.execute('update WORK set JUDGE="审批通过" where PROJECTWORKER="%s" and MONDAYTIME="%s" and PROJECTMANAGER="%s" and PROJECTID="%s" and JUDGE="未审批" '%(businessman,mondaytime,username,businesscode))
db.commit()
